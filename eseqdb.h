...
public:
  ...
  //  ediag(int _i,int _j,int _i2,int _j2):
  //  i(_i),j(_j),i2(_i2),j2(_j2),V(-1),bestseg(0x00) {}
  ediag(long _i, long _delta, long _len)
      : i(_i), j(_i + _len), i2(_i + _delta), j2(_i + _delta + _len), V(-1),
        bestseg(0x00) {}
};
...
typedef estr (*outfmt_fd)(const etax &, const earrayof<double, int> &,
                          const efloatarray &);
...
public:
  ...
  void init(const estr &dbname, bool nocluster = false,
            outfmt_fd ofmt = outfmt_confidences);

  void seqsearch(const estr &str2id, eseq &s, earray<epredinfo> &pinfoarr,
                 esearchws &sws);
  void seqsearch_global(const estr &str2id, eseq &s,
                        earray<epredinfo> &pinfoarr, esearchws &sws);
  void seqsearchpair(const estr &id, eseq &s, eseq &s2,
                     earray<epredinfo> &pinfoarr, esearchws &sws);
  void pseqsearch(const estr &str2id, eseq &s, earray<epredinfo> &pinfoarr,
                  esearchws &sws);

  void seqalign_global(const estr &str2id, eseq &s,
  ...
  int processQueryPairend(const estr &fname, const estr &fname2,
                          void (*taskfunc)(), ethreads &t);
  int processQueryPairendFASTQ(const estr &fname, const estr &fname2,
                               void (*taskfunc)(), ethreads &t);
